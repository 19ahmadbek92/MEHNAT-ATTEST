<x-app-layout>
    <x-slot name="header">➕ Yangi attestatsiya kampaniyasi</x-slot>

    <div style="max-width: 680px;">
        <div class="att-card">
            <div style="background: var(--ink); padding: 20px 24px; border-radius: 12px 12px 0 0;">
                <h3 style="font-family: 'DM Serif Display', serif; font-size: 20px; color: white; margin: 0 0 4px;">Yangi kampaniya yaratish</h3>
                <p style="color: rgba(255,255,255,0.4); font-size: 13px; margin: 0;">Barcha maydonlarni to'ldiring</p>
            </div>
            <div class="att-card-body">
                <form method="POST" action="{{ route('admin.campaigns.store') }}">
                    ...
                    <div class="att-field" style="margin-bottom: 18px;">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required placeholder="Masalan: 2026-yil bahorgi attestatsiya">
                        @error('title') <p style="color: var(--red); font-size: 12px; margin-top: 4px;">{{ $message }}</p> @enderror
                    </div>

                    <div class="att-field" style="margin-bottom: 18px;">
                        <label>Izoh</label>
                        <textarea name="description" rows="3" placeholder="Kampaniya haqida qisqacha izoh...">{{ old('description') }}</textarea>
                    ...
                    <div class="att-field" style="margin-bottom: 24px;">
                        ...
                        <select name="status">
                            ...
                            <option value="open" @selected(old('status') === 'open')>✅ Ochiq</option>
                            <option value="closed" @selected(old('status') === 'closed')>🔒 Yopilgan</option>
                        </select>
